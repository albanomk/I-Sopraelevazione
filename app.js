...
(() => {
  ...
  function updateReadyState() {
    ...
    ui.statusLed.classList.remove('done');
    if (ready) {
  ...
  async function loadPdf(file) { return await pdfjsLib.getDocument({ data: await fileToArrayBuffer(file) }).promise; }
  ...
  async function extractProjectRows(targetMin,targetMax,trackMode){
    ...
    try{
      for(let f=0;f<state.projectFiles.length;f++){
        const file=state.projectFiles[f]; const pdf=await loadPdf(file); setProgress(31,`Individuazione pagine di progetto · ${file.name}`);
  ...
  async function analyze() {
    ...
    try{
      ...
      for(let i=0;i<state.graphFiles.length;i++){
        const f=state.graphFiles[i]; const one=f.type==='application/pdf'||/\.pdf$/i.test(f.name)?await analyzeGraphPdf(f,sampleStep):await analyzeGraphImage(f,sampleStep); graphSamples.push(...one);
      }
      ...
      setProgress(100,'Analisi completata'); renderResults(); ui.statusTitle.textContent='Analisi completata'; ui.statusText.textContent=`${state.alerts.length} curva/e aggregate da verificare · affidabilità ${state.reliability.score}%.`; ui.statusLed.classList.remove('ready'); ui.statusLed.classList.add('done'); setTimeout(hideProgress,900);
    } catch(err){ console.error(err); ui.statusTitle.textContent='Analisi non completata'; ui.statusText.textContent=err.message||'Errore durante l’analisi.'; state.diagnostics.push(err.stack||String(err)); setProgress(100,'Analisi interrotta'); }
  ...
  function exportPdf(){
    ...
    doc.setFont('helvetica','bold');doc.setFontSize(18);doc.text('I-Sopraelevazione · Report aggregato',14,15);
    ...
    legend.forEach(([label,l])=>{doc.setFillColor(COLORS[l]);doc.roundedRect(x,35,23,7,2,2,'F');doc.setTextColor(l==='yellow'?20:255);doc.setFontSize(8);doc.setFont('helvetica','bold');doc.text(label,x+11.5,39.7,{align:'center'});x+=27;});
  ...
  function loadDemoPreview(){
    ...
    state.samples=Array.from({length:120},(_,i)=>({confidence:.91+(i%7)*.008}));
