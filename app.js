/* I-Sopraelevazione v1.4.1 - static GitHub Pages app
...
(() => {
  ...
  async function analyzeGraphPdf(file, sampleStep, trackMode) {
    ...
    for (let pageNo = 1; pageNo <= pdf.numPages; pageNo++) {
      ...
      // Si segue fisicamente la linea da sinistra a destra, indipendentemente dal verso delle progressive.
      points.sort((a,b)=>a.x-b.x);
      const traced = traceSuperelevationSeries(image, canvas.width, canvas.height, points, zeroY, mmPerPixel, xMin, xMax);
      let bridged = 0, skipped = 0;
      for (const t of traced) {
        ...
        out.push({ source:file.name, page:pageNo, pk:t.pk, measured, signed:(zeroY-t.y)*mmPerPixel, confidence:t.confidence, traceMethod:t.method || 'tracked' });
      }
      if (bridged || skipped) {
        state.diagnostics.push(`${file.name} p.${pageNo}: inseguimento linea sopraelevazione · ${bridged} campione/i ricostruiti per continuità, ${skipped} non affidabili esclusi (mai forzati a 0).`);
      }
    }
    return out;
  }
  ...
  // Media dei pixel più scuri in un piccolo intorno 2D. A differenza della vecchia
  // media orizzontale non favorisce le griglie e riconosce meglio una linea inclinata.
  function localStrokeGray(data, width, height, x, y) {
    ...
    vals.sort((a,b)=>a-b);
    const take=Math.min(6,vals.length); let sum=0;
    for(let i=0;i<take;i++)sum+=vals[i];
    return sum/take;
  }

  // Una riga molto scura per gran parte della larghezza è probabilmente una griglia,
  // la linea di fede o un'altra linea orizzontale. Non viene esclusa, ma penalizzata.
  function buildRowInkDensity(data, width, height, yMin, yMax, xMin, xMax) {
    const density = new Float32Array(height);
    const stepX = Math.max(7, Math.round((xMax-xMin)/160));
    for(let y=Math.max(1,Math.floor(yMin)); y<=Math.min(height-2,Math.ceil(yMax)); y++){
      ...
      for(let x=Math.max(1,Math.floor(xMin));x<=Math.min(width-2,Math.ceil(xMax));x+=stepX){
        if(grayAt(data,width,height,x,y)<205)dark++;
        n++;
      }
      density[y]=n?dark/n:0;
    }
    return density;
  }

  function traceCandidateAtX(data, width, height, x, zeroY, prevY, prevPrevY, expectedPx, rowDensity, maxOffsetPx) {
    const yMin=Math.max(2,Math.round(zeroY-maxOffsetPx));
    const yMax=Math.min(height-3,Math.round(zeroY+maxOffsetPx));
    const prevFinite=Number.isFinite(prevY), prevPrevFinite=Number.isFinite(prevPrevY);
    const prevSlope=prevFinite&&prevPrevFinite?clamp(prevY-prevPrevY,-14,14):0;
    const predicted=prevFinite?clamp(prevY+prevSlope,yMin,yMax):zeroY;
    const prevOffset=prevFinite?Math.abs(prevY-zeroY):0;
    let best=null;

    for(let y=yMin;y<=yMax;y++){
      const ink=localStrokeGray(data,width,height,x,y);
      // Su fondo bianco non inventiamo una misura: se la linea non è leggibile il campione sarà escluso/ricostruito.
      if(ink>243)continue;
      const offset=Math.abs(y-zeroY);
      const continuityCost=prevFinite?Math.abs(y-predicted)*0.82:0;
      const accelCost=prevFinite&&prevPrevFinite?Math.abs((y-prevY)-prevSlope)*0.34:0;
      const projectCost=Number.isFinite(expectedPx)?Math.abs(offset-expectedPx)*0.075:0; // solo indizio debole
      const horizontalPenalty=Math.max(0,(rowDensity[Math.round(y)]||0)-0.16)*92;
      let zeroPenalty=0;
      if(offset<3){
        // La fede non deve attirare il tracciatore quando la curva che stavamo seguendo era altrove.
        if(prevOffset>7)zeroPenalty+=125;
        else if(Number.isFinite(expectedPx)&&expectedPx>10)zeroPenalty+=32;
        else zeroPenalty+=8;
      }
      const jump=Math.abs(y-(prevFinite?prevY:y));
      const jumpPenalty=prevFinite&&jump>34?(jump-34)*1.8:0;
      const score=ink+continuityCost+accelCost+projectCost+horizontalPenalty+zeroPenalty+jumpPenalty;
      if(!best||score<best.score)best={y,score,ink,continuityCost,horizontalPenalty,offset};
    }
    if(!best)return null;
    // Salti molto grandi sono ammessi solo se la traccia è nettissima; la sopraelevazione reale è geometricamente continua.
    if(prevFinite&&Math.abs(best.y-prevY)>58&&best.ink>150)return null;
    const inkQ=clamp((243-best.ink)/105,0,1);
    const contQ=prevFinite?clamp(1-Math.abs(best.y-predicted)/34,0,1):0.72;
    const gridQ=clamp(1-best.horizontalPenalty/100,0.25,1);
    const confidence=clamp(0.42+inkQ*0.34+contQ*0.18+gridQ*0.06,0.42,0.98);
    return {y:best.y,confidence,method:'tracked'};
  }

  function repairTraceGaps(rows, zeroY) {
    const out=rows.map(r=>({...r}));
    const n=out.length;
    // 1) Piccoli vuoti: interpolazione soltanto se la traccia è presente sui due lati.
    for(let i=0;i<n;){
      if(Number.isFinite(out[i].y)){i++;continue;}
      const start=i; while(i<n&&!Number.isFinite(out[i].y))i++; const end=i-1;
      const left=start-1,right=i;
      const len=end-start+1;
      if(left>=0&&right<n&&len<=4&&Number.isFinite(out[left].y)&&Number.isFinite(out[right].y)&&Math.abs(out[right].y-out[left].y)<=48){
        for(let k=start;k<=end;k++){
          const t=(k-left)/(right-left);
          out[k].y=out[left].y+(out[right].y-out[left].y)*t;
          out[k].confidence=clamp(Math.min(out[left].confidence||.7,out[right].confidence||.7)*.72,.45,.68);
          out[k].method='bridge-gap';
        }
      }
    }
    // 2) Cadute isolate sulla fede: se la linea continua chiaramente prima e dopo,
    // il tratto a zero è considerato una sovrapposizione/intersezione e non un vero H=0.
    for(let i=0;i<n;){
      if(!Number.isFinite(out[i].y)||Math.abs(out[i].y-zeroY)>=3){i++;continue;}
      const start=i; while(i<n&&Number.isFinite(out[i].y)&&Math.abs(out[i].y-zeroY)<3)i++; const end=i-1;
      const left=start-1,right=i,len=end-start+1;
      if(left>=0&&right<n&&len<=4&&Number.isFinite(out[left].y)&&Number.isFinite(out[right].y)){
        const lo=out[left].y-zeroY, ro=out[right].y-zeroY;
        const sameSide=lo*ro>0;
        const safelyAway=Math.abs(lo)>7&&Math.abs(ro)>7;
        const smooth=Math.abs(out[right].y-out[left].y)<=42;
        if(sameSide&&safelyAway&&smooth){
          for(let k=start;k<=end;k++){
            const t=(k-left)/(right-left);
            out[k].y=out[left].y+(out[right].y-out[left].y)*t;
            out[k].confidence=clamp(Math.min(out[left].confidence||.7,out[right].confidence||.7)*.70,.45,.66);
            out[k].method='bridge-overlap';
          }
        }
      }
    }
  ...
  function traceSuperelevationSeries(data, width, height, points, zeroY, mmPerPixel, xMin, xMax) {
    ...
    // Copertura fino a circa 190 mm: la vecchia finestra era troppo stretta e poteva perdere quote elevate.
    const maxOffsetPx=Math.max(80,Math.ceil(190/Math.max(.01,mmPerPixel))+8);
    const rowDensity=buildRowInkDensity(data,width,height,zeroY-maxOffsetPx,zeroY+maxOffsetPx,xMin,xMax);
    const raw=[];
    let prevY=null,prevPrevY=null;
    for(const p of points){
      const expectedPx=Number.isFinite(p.project)?Math.abs(p.project)/mmPerPixel:null;
      const c=traceCandidateAtX(data,width,height,p.x,zeroY,prevY,prevPrevY,expectedPx,rowDensity,maxOffsetPx);
      if(!c){raw.push({...p,y:null,confidence:.35,method:'missing'});continue;}
      raw.push({...p,...c});
      prevPrevY=prevY; prevY=c.y;
    }
    return repairTraceGaps(raw,zeroY);
  }

  // Mantenuta per compatibilità con eventuali chiamate residue. Non restituisce più 0
  // quando perde la linea: in quel caso restituisce null.
  function traceAtX(data, width, height, x, zeroY, prevY, scale) {
    const mmPerPixel=(25.4/72/Math.max(1,scale))*6.0;
    const maxOffsetPx=Math.max(80,Math.ceil(190/Math.max(.01,mmPerPixel))+8);
    const density=buildRowInkDensity(data,width,height,zeroY-maxOffsetPx,zeroY+maxOffsetPx,0,width-1);
    const c=traceCandidateAtX(data,width,height,x,zeroY,prevY,null,null,density,maxOffsetPx);
    return c?c.y:null;
  }

  function traceConfidence(data, width, height, x, y, zeroY) {
    if(!Number.isFinite(y))return .35;
    const g=localStrokeGray(data,width,height,x,y);
    return clamp(.45+(243-g)/210,.45,.98);
  }
  ...
  async function analyze() {
    ...
    try{
      ...
      state.diagnostics.push(`Campioni grafico affidabili: ${graphSamples.length}. Campioni confrontati: ${state.samples.length}. Il tracciatore segue la continuità della linea e non converte più una perdita di traccia in H=0.`);
